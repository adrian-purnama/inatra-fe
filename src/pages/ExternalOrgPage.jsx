...
import { ExternalOrgForm } from "../forms/ExternalOrgForm.jsx";
import { apiDelete, apiGet, paths } from "../lib/api.js";

export function ExternalOrgPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    setErr("");
    try {
      const res = await apiGet(paths.opportunityExternalOrg);
      setRows(res?.data?.items ?? []);
    } catch (e) {
      setErr(e?.message ?? "Failed to load external orgs");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  async function handleDelete(item) {
    ...
    try {
      await apiDelete(`${paths.opportunityExternalOrg}/${item.id}`);
      await loadItems();
    } catch (e) {
      setErr(e?.message ?? "Failed to delete external org");
    } finally {
      setDeletingId("");
    }
  }
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
            Manage external organizations for opportunity data entry.
          </p>
        </div>
        <button
          type="button"
      ...
      <DataTable
        ...
        loading={loading}
        error={err || null}
        emptyMessage='No external org rows yet. Click "Create external org".'
        actions={(row) => (
