...
import { MarketSegmentForm } from "../forms/MarketSegmentForm.jsx";
import { apiDelete, apiGet, paths } from "../lib/api.js";
import { DataTable } from "../components/DataTable.jsx";
import { Modal } from "../components/Modal.jsx";
import { useUser } from "../context/UserContext.jsx";

export function MarketSegmentPage() {
  ...
  const loadItems = useCallback(async () => {
    ...
    setErr("");
    try {
      const res = await apiGet(paths.opportunityMarketSegment);
      setRows(res?.data?.items ?? []);
    } catch (e) {
      setErr(e?.message ?? "Failed to load market segment");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  async function handleDelete(item) {
    ...
    try {
      await apiDelete(`${paths.opportunityMarketSegment}/${item.id}`);
      await loadItems();
    } catch (e) {
      setErr(e?.message ?? "Failed to delete market segment");
    } finally {
      setDeletingId("");
    }
  }
  ...
  return (
    <div className="w-full">
      ...
      <DataTable
        ...
        loading={loading}
        error={err || null}
        emptyMessage='No market segment rows yet. Click "Create market segment".'
        actions={(row) => (
