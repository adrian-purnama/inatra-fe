...
/**
 * @param {object} props
 * @param {{ id?: string; name?: string; description?: string; isActive?: boolean } | null} [props.initial]
 * @param {() => Promise<void> | void} props.onSuccess
 * @param {() => void} props.onCancel
 */
export function ExternalOrgForm({ initial = null, onSuccess, onCancel }) {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (isEdit) {
        await apiPatch(`${paths.opportunityExternalOrg}/${initial.id}`, body);
      } else {
        await apiPost(paths.opportunityExternalOrg, body);
      }
      await onSuccess();
    } catch (err) {
      setFormErr(err?.message ?? "Could not save external org");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        >
          {submitting ? "Saving..." : isEdit ? "Save external org" : "Create external org"}
        </button>
        <button
          type="button"
          disabled={submitting}
