 ...
 */
export function MarketSegmentForm({ initial = null, onSuccess, onCancel }) {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      if (isEdit) {
        await apiPatch(`${paths.opportunityMarketSegment}/${initial.id}`, body);
      } else {
        await apiPost(paths.opportunityMarketSegment, body);
      }
      await onSuccess();
    } catch (err) {
      setFormErr(err?.message ?? "Could not save market segment");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        >
          {submitting ? "Saving..." : isEdit ? "Save market segment" : "Create market segment"}
        </button>
        <button
          type="button"
          disabled={submitting}
